...
class TiniEnvironmentConfig {
    ...
    async loadConfigFromAPI() {
        try {
            // Chỉ fetch nếu cấu hình BASE URL được cung cấp rõ ràng từ window.TINI_CONFIG.API_BASE
            const baseFromWindow = (typeof window !== 'undefined' && window.TINI_CONFIG && typeof window.TINI_CONFIG.API_BASE === 'string')
                ? window.TINI_CONFIG.API_BASE.trim()
                : '';

            if (!baseFromWindow) {
                console.log('ℹ️ Skip remote config fetch (no TINI_CONFIG.API_BASE set)');
                return;
            }

            const apiBase = baseFromWindow.replace(/\/$/, '');
            const url = `${apiBase}/api/config/client`;
            const response = await fetch(url, { mode: 'cors' });
            if (response.ok) {
                const apiConfig = await response.json();
                this.config = { ...this.config, ...apiConfig };
                console.log('✅ Client config loaded from API', url);
            } else {
                console.warn('⚠️ Config API responded non-OK:', response.status, url);
            }
        } catch (error) {
