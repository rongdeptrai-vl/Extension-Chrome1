...
const ADMIN_PORT_FALLBACKS = (process.env.ADMIN_PORT_FALLBACKS || '3005,3006')
  .split(',')
...
const authenticateToken = async (req, res, next) => {
    try {
        const user = await SecurityCore.authenticateRequest(req);
        ...
        const hasPermission = await SecurityCore.checkPermission(
            user,
            req.path,
...
async function startServer() {
    try {
        ...
        const candidates = Array.from(new Set([parseInt(ADMIN_PORT,10), ...ADMIN_PORT_FALLBACKS]));
        let i = 0;

        const listenNext = () => {
            const p = candidates[i];
            server.once('error', (err) => {
                if (err && err.code === 'EADDRINUSE' && i < candidates.length - 1) {
                    console.warn(`[admin-dashboard] Port ${p} in use, trying ${candidates[i+1]}...`);
                    i++;
                    listenNext();
                } else {
            ...
            server.listen(p, () => {
                console.log(`🚀 Admin Dashboard Server running on http://localhost:${p}`);
